...
function completarConfig(divConfig) {
    ...
    let tablaOpcionales = document.querySelector("#opcionales");
    let tablaBasica = document.querySelector("."+divConfig+" table");
    tablaBasica.innerHTML += tablaOpcionales.innerHTML;
    let btnConfig = tablaBasica.querySelector("#btnConfig");
...
function colorElegido(color) {
    ...
    let divTexto = sessionStorage.getItem("divTexto");
    let tablaConfig = document.querySelector("."+divTexto);
    let divColorCarroceria = tablaConfig.querySelector("#colorCarroceria");
...
function guardar() {
    ...
    // <span onclick="borrarConfig(idConfiguracion)">X</span>`;
    refConfig.innerHTML = `Configuración #${idConfiguracion}: ${autoMarca(divTexto)} 
    <div class="contboton" style="height: 15px;"><a class="boton" onclick="borrarConfig(idConfiguracion)" style="font-size: 20px;">Borrar</a></div>`;
